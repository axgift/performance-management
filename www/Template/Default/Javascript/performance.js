    ...
    const WorkPlanApp = {
       ...
       $('.chart-placeholder').css('display', 'none');
        ...
        handleEmployeeSelect: function(e) {
            ...
            var plans=[];
            $("#performancelist").find("ul").empty();

            $.ajax({
                type: "POST",
                url: "/AjaxFun/post.ashx",
                data: { r: "getuserinfobyid",id:userid},
                datatype: "json",
                beforeSend: function () {
                ...
                success: function (_data) {
                    ...
                    if (unescape(data.code) == "1") {
                        ...
                        $("#mainperformance h3").html("期数 ("+returnMsg.split('|')[0]+")");
                        $("#mainperformance label").html("评分 ("+returnMsg.split('|')[1]+")");
                        ...
                        //显示最近3个计划
                                ...
                                recentPlans.forEach(week => {
                                    const historyItem = $(`
                                        <li><label for="score">${week.Plan_ID} 评分 (${week.rating})</label></li>
                        `);


                       
                        $("#performancelist").find("ul").append(historyItem);
