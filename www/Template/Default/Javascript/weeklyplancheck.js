$(document).ready(function() {
    ...
    const WorkPlanApp = {
            ...
            $.ajax({
                ...
                success: function (_data) {
                    ...
                    if (unescape(data.code) == "1") {
                        ...
                        plans.sort().reverse();
                        // 显示最近3个计划
                        const recentPlans = plans.slice(0, 15);
                        recentPlans.forEach(week => {
                            const historyItem = $(`
        ...
        handleEmployeeSelect: function(e) {
            ...
            var _this=this;
            var plans=[];
            $("#mainperformance").find("ul").empty();

            $.ajax({
                type: "POST",
                url: "/AjaxFun/post.ashx",
                data: { r: "getweekplanbyuserid",id:userid,week:$('#plan-week').val()},
                datatype: "json",
                beforeSend: function () {
                ...
                success: function (_data) {
                    ...
                    if (unescape(data.code) == "1") {
                        ...
                        $("#mainperformance h3").html("期数 ("+plans[0].Plan_ID+")"+(unescape(data.message)=="0"?"<span class=\"bohui\">已驳回</span>":""));
                        ...
                        recentPlans.forEach(week => {
                            const historyItem = $(`
                                <li class="nole"><div style="float:left;">${ unescape(week.Plan_Note)}</div></li>
                        `);                       
                        ...
                        $('#Reject_btn').on('click', function(){
                            ...
                            var userID=$("#User_list").find(".active").attr("data-value");
                            var weekid=$('#plan-week').val();

                            $.getJSON("/AjaxFun/post.ashx?r=updateweekplanstatus&week=" + weekid + "&userid=" + userID + "&rs=" + Math.floor(Math.random() * 1000), function (data) {
